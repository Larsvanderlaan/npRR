...
lrnr_stack <-  Stack$new(list( Lrnr_glmnet$new() ,
            ...
            Lrnr_hal9001$new(max_depth = 2, smoothness_orders = 1, num_knots = c(20,1))))
...
lrnr_superLearner_A <- make_learner(Pipeline, lrnr_cv_stack, Lrnr_nnls$new())
sl3_Learner_pA1W <- lrnr_superLearner_A

lrnr_cv_stack <- Lrnr_cv$new(lrnr_stack_stratified)
lrnr_superLearner_Y <- make_learner(Pipeline, lrnr_cv_stack, Lrnr_nnls$new())
sl3_Learner_EYAW <- lrnr_superLearner_Y
