...
estimate_LRR_using_ERM <- function(W, A, Y,  EY1, EY0, pA1, weights, sl3_LRR_Learner_binomial, learning_method = c("plugin", "IPW"), Wpred = W) {
  ...
  if(learning_method == "plugin") {
    ...
    pseudo_outcome <- A
    pseudo_weights <- Y / ifelse(A==1, pA1, 1 - pA1)
    data$pseudo_outcome <- pseudo_outcome
    data$pseudo_weights <- pseudo_weights
    task_LRR <- sl3_Task$new(data, covariates = covariates, outcome = "pseudo_outcome", weights = "pseudo_weights", outcome_type = "binomial")
  }
...
DR_risk_function_LRR <- function(LRR, A, Y, EY1, EY0, pA1, weights, debug = FALSE, return_loss = FALSE) {
  LRR <- as.matrix(LRR)
  if(!(nrow(LRR) == length(A) && length(LRR) == length(EY1))) {
...
lrnr <- Lrnr_cv$new(Lrnr_xgboost$new(max_depth = 5))
likelihood <- estimate_initial_likelihood(W, A, Y, weights, lrnr, lrnr)
...
list_of_learners <- list(
  Lrnr_hal9001$new(max_degree = 1, smoothness_orders = 1, num_knots = 2, lambda = c(0.0001, 0.0005, 0.001, 0.1), fit_control = list(cv_select = FALSE) , family = binomial())
)
...
lapply(LRR_learners_by_fold, function(item) {
  names(item) <- c("LRR_train", "LRR_pred", "LRR_learner")
})
...
output <- lapply(seq_along(folds), function(fold_number) {
  ...
  LRR_learners <- lapply(learner_names, function(learner_name) {
      keep <- which(stringr::str_detect(names(LRR_learners), learner_name))
      ...
      best_index <- as.numeric(seq_along(all_training_risks) == which.min(all_training_risks))
