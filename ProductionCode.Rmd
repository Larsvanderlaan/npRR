...
estimate_LRR_using_ERM <- function(W, A, Y,  EY1, EY0, pA1, weights, sl3_LRR_Learner_binomial, learning_method = c("plugin", "IPW"), Wpred = W) {
  ...
  output <- list(LRR_train = LRR, LRR_pred = LRR_pred, LRR_learner = sl3_Learner_LRR_trained)
  return(output)
}
...
output <- lapply(seq_along(folds), function(fold_number) {
  ...
  LRR_learners <- lapply(learner_names, function(learner_name) {
      ...
      all_training_risks <- lapply(all_LRR_training, function(LRR) {
