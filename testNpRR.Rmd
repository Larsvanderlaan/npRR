...
```{r}
 n <- 2500
  ...
  fit_RR <- npRR(list(Lrnr_gam$new(),Lrnr_glmnet$new(lambda = c(1e-9, 1e-4, 1e-3))), W= W, A = A, Y = Y, V = W, weights = weights, sl3_Learner_EYAW = sl3_Learner_EYAW, sl3_Learner_pA1W = sl3_Learner_pA1W, outcome_type = "continuous", list_of_sieves = list_of_sieves,cross_validate_LRR = TRUE) 
...
data.table(preds)
data.table(LRR, preds , do.call(cbind, lapply(fit_RR$LRR_learners, `[[`,"LRR_pred")))
```
